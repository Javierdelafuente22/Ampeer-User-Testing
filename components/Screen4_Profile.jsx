...
function Screen4_Profile({ state, setState, onNext, onBack, fullName = 'Sarah Chen', initials = 'SC' }) {
  ...
  const facts = state.profile || [
    { id: 'name',   icon: <IconHome size={16}/>,      label: 'Name',            value: fullName,                   why: "It appears on your dashboard and weekly reports" },
    ...
    <PwScreen step={3} onBack={onBack}>
      ...
      <div className="pw-card" style={{ marginTop: 24, overflow: 'visible' }}>
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--ink-900)', letterSpacing: '-0.005em' }}>{fullName}</div>
            <div style={{ fontSize: 12, color: 'var(--ink-600)' }}>
              {state.postcode || 'SW6 3JD'} · {state.area || 'London'}
            </div>
          </div>
          <div style={{
